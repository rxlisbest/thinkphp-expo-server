...
var ws = new WebSocket("ws://39.96.71.175:2346");
// ws.onopen = function () {
...
function init(joystick, eee) {
  ...
  ji.src = '../test/img/joystickin.png'; //加载图片
  jo.src = '../test/img/joystickout.png'; //加载图片
      ...
      switch (event.type) {
        ...
        case "touchend": //手指离开的时候
          ...
          // for (var i = 0; i < ef.length; i++) {
					// 	if (Math.sqrt(Math.pow(ef[i].clientX - centerX - touchX, 2) +
					// 		Math.pow(eef[i].clientY - centerY - touchY, 2)) <=
					// 		josize / 2 - jisize / 2) {
					// 		continue
					// 	} else {
					// 		jx = 0;
